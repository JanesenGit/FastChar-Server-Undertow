...
public class FileSessionPersistence implements SessionPersistenceManager {
	...
	private File getSessionFile(String deploymentName) {
		if (!this.dir.exists()) {
			if (!this.dir.mkdirs()) {
				throw new IllegalStateException("Failed to create directory: " + this.dir.getAbsolutePath());
			}
		}
	...
	public void clear(String deploymentName) {
		if (!getSessionFile(deploymentName).delete()) {
			FastChar.getLogger().warn("Failed to clear persistent session: " + deploymentName);
		}
	}
